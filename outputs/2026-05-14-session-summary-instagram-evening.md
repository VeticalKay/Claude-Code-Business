# Session 14.05.2026 (Abend) — Instagram Performance + BTS-Reel-Konzept

## Was wurde besprochen

1. **Founder Story Karussell** ist live gegangen auf @vertical_kay_pole
2. **Performance-Analyse** der ersten 22 Stunden anhand Instagram-Insights
3. **BTS-Reel-Konzept** für Kraftübungs-Filming-Chaos ausgearbeitet (10 Szenen)

## Was wurde erledigt

### 🎉 Founder Story Karussell LIVE
- Post: https://www.instagram.com/p/DYSzhd9gr2o/
- Co-Branded mit @verticalkay_pole_education
- Standort: Bergheim, Salzburg, Austria
- 10 Slides Aubergine Studio Brand, bilinguale Caption, Top 5 Hashtags

### 📊 Performance-Analyse (nach 22h)
| Metrik | Wert | Bewertung |
|---|---|---|
| Aufrufe | 547 | OK (40% Reach) |
| Erreichte Konten | 175 | OK |
| Likes | 17 | Niedrig |
| Kommentare | **0** | ⚠️ CTA hat nicht gegriffen |
| Saves | **0** | ⚠️ Niemand fand es speicherwürdig |
| Shares | **0** | ⚠️ Niemand wollte teilen |
| Profilbesuche | **0** | ⚠️ Karussell zog nicht aufs Profil |
| Neue Follower | **0** | ⚠️ Keiner blieb hängen |
| Non-Follower-Reach | 7,8% | ⚠️ Algorithmus pusht es kaum raus |
| Facebook | 88 Aufrufe, 3 Reaktionen | OK |

**Erkenntnis:** Erster Post unter neuer Strategie hat Brand-Building geleistet, aber strategisch wenig bewirkt. Founder Stories sind oft so — wenig Engagement, aber wertvoll für Brand-Aufbau. **Karussell 2 muss strategisch anders sein.**

### 🎬 BTS-Reel Konzept (Behind-the-Scenes HOME WORKOUT Filming — FINALE VERSION 15.05.2026)
**Wichtig:** Es geht um Kraftübungen für ZUHAUSE — unabhängig ob man eine Pole hat oder nicht. Für Schülerinnen die zwischen Studio-Terminen Kraft aufbauen wollen.

**FINALE Reihenfolge (10 Szenen, 30 Sek) — STÄRKT + blblbl früh, Pistol-Puch als später Energie-Boost vor Outro:**

| Sek | # | Szene | English Text | Funktion |
|---|---|---|---|---|
| 0:00 | 1 | Handy quietscht, dreht | (Hook): "POV: the chaos behind every clean tutorial." | HOOK |
| 0:02 | 2 | Lautes "STÄRKT!" | "When the word just won't come out." | PEAK 1 |
| 0:05 | 3 | Voice: "Ich krieg die Krise" (Matte) | "Forgot to press record. Again." | Setup-Voice |
| 0:08 | 4 | "blblblblbl" | "When the brain leaves the building." | PEAK 2 |
| 0:11 | 5 | Frame-Check Arme | "Trying not to crop my head off." | Verschnauf |
| 0:14 | 6 | Voice: "Nein das darf nicht sein" (Matte) | "No. Not again." | Eskalation |
| 0:17 | 7 | Krampf | "And now: a cramp." | Körperlicher Tiefpunkt |
| 0:20 | 8 | Stumm kniend, Kopf in Händen | "Take 47." | Emotionaler Tiefpunkt |
| 0:23 | 9 | Pistol-Puch (rechts/links) — LATE TWIST | "Right? Left? *gives up*" | PEAK 3 — Energie-Boost vor Outro |
| 0:26 | 10 | Voice: "Wurm drin" (Matte) | "Nothing's working today." | OUTRO |

**Alte 10-Szenen-Reihenfolge (veraltet, durch obige ersetzt):**
1. Handy quietscht / Bild dreht (HOOK)
2. Voice: "Recording vergessen"
3. Voice: "Kopf abgeschnitten"
4. Arme Frame-Check
5. Pistolen-Puch (rechts/links verheddert)
6. Lautes "STÄRKT!" (PEAK 1)
7. "blblblbl" (PEAK 2 — von 9 vorgezogen)
8. Krampf (von 10 vorgezogen)
9. Stumm verzweifelt kniend
10. "Da ist der Wurm drin" (wholesomer OUTRO)

**Empfehlung:**
- **Hook (on-screen):** **"5 Minuten. 47 Versuche."** (Top-Pick — kurz, kontrastreich, Numbers stoppen) — Alternativen: "Du siehst 5 Minuten. Ich sehe das.", "Was niemand zeigt.", "Heute habe ich 47 Mal aufgegeben."
- **Szene 3 Klarstellung:** voice-only "Kamera zu nah, Kopf gerade noch im Bild" (NICHT "abgeschnitten") → On-Screen-Text: "Kamera zu nah. Kopf gerade noch da."
- **Pro Szene:** 3-5 Wörter Text auf Deutsch (für Stumm-Schauer)
- **Länge:** 36 → 30 Sek kürzen für Retention
- **Caption:** "5 Min Tutorial = 5 Stunden dahinter..." (komplett ausgearbeitet, EN + DE)
- **Hashtags:** #homeworkout #polestrength #poletraining #behindthescenes #verticalkay
- **Caption-Pointe:** "I create these workouts so my students (and you) can build pole strength at home — pole or no pole." / "Ich mache diese Workouts, damit meine Schülerinnen Pole-Kraft auch zuhause aufbauen können — mit oder ohne Pole."

**Schrift/Style-Specs für On-Screen-Texte:**
- **Hook:** Montserrat Bold, 60-80px, Weiß #FFFFFF, zentriert obere Hälfte, dezenter Outline für Lesbarkeit
- **Szenen-Texte:** Montserrat Regular, 40-55px, Weiß #FFFFFF (oder Sand #e0dbd9 für softer), untere Hälfte oberhalb Instagram-UI
- **Dauer:** 2-3 Sek pro Text-Einblendung
- **KEIN Magenta** für Texte (Education Brand würde brechen)
- **Aubergine #702f70** optional als Outro-Akzent
- **Musik:** Trending BTS/Fail-Audio (kein Klagelied)

**Ziel:** Authentizität zeigen, dass das Filmen Wahnsinn ist und man dranbleiben muss. Heiter, sympathisch, menschlich-nahbar.

## Offene Punkte / Nächste Schritte

### Morgen (15.05.2026)
1. **BTS-Reel zusammenschneiden** in CapCut/InShot — Reihenfolge wie empfohlen, Hook-Text + Szene-Texte einfügen, auf 30 Sek bringen, trending BTS-Musik
2. **Karussell 2 — The Method** strategisch anders aufbauen (siehe nächster Punkt)

### Karussell 2 Strategie-Anpassung (NICHT VERGESSEN!)
Nach Founder-Story-Erkenntnissen muss Karussell 2 (The Method, 4 Säulen) anders gebaut werden:
- **Hook:** Curiosity-provozierend, nicht poetisch-aufbauend (z.B. "Diese 4 Säulen ändern wie du Pole erlebst")
- **Save-Trigger:** "Save this for when you forget why you started"
- **Comment-Frage:** einfacher mit niedriger Hürde — "Welche der 4 Säulen trifft dich am meisten? Schreib 1, 2, 3 oder 4"
- **Hashtags:** breiter mixen für mehr Non-Follower-Reach
- **Caption:** kürzer, kommentar-provozierender (nicht so poetisch wie Founder)

### Performance-Hebel für Founder-Post (nächste 24-48h)
- Story-Push mit Slide 1 + Pfeil zum Karussell
- DM an 5-10 engste Schülerinnen für Feedback (boostet Algorithmus nachträglich)
- Karussell als 15-Sek Reel re-purposen (anderer Algorithmus-Pfad)

---

_Dauer: kurze Abend-Session_
_Status: Reel-Konzept fertig, BTS-Reel-Produktion morgen_
